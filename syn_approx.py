...
def main(dataset, d, N_R, sig_frac):
    ...
    for name in datasets:
        ...
        if compute_err:
            ...
            with open(directory + 'err_bin/' + name + dim_str + sigma_str + N_R_str, 'wb') as f:
                pkl.dump(errs, f)
            print('saved errs')
        else:
            with open(directory + 'err_bin/' + name + dim_str + sigma_str + N_R_str, 'rb') as f:
                errs = pkl.load(f)
                print('load errs')
        ...
        plt.tight_layout()
        plt.savefig('plottings/syn_approx_' + dim_str + sigma_str + N_R_str + '.png', format='png', dpi=200)
        plt.show()
...
if __name__ == "__main__":
    ...
    N_R = sys.argv[3]
    sig_frac = sys.argv[4]
    main(dataset, int(d), int(N_R), float(sig_frac))